...
export default function AnimatedHeader() {
  ...
  const navLinks = ["Home", "Services", "About", "Emergency", "Support", "Contact"]; // Add this too
  const headerClasses = `fixed top-0 left-0 right-0 z-50 border-b shadow-lg ${isHomePage ? 'bg-white dark:bg-slate-900' : 'bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl'} border-slate-200/50 dark:border-slate-700/50`;
        ...
        {mobileMenuOpen && (
          ...
          >
            <div className="flex flex-col space-y-4 pt-4">
              {["Services", "About", "Emergency", "Support", "Contact"].map(
                (item) => (
                  <Link
                    key={item}
